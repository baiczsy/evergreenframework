...
import org.evergreen.web.utils.beanutils.expression.DefaultResolver;
import org.evergreen.web.utils.beanutils.expression.Resolver;
import org.evergreen.web.utils.collections.FastHashMap;
...
public class PropertyUtilsBean {
    ...
    public Object getSimpleProperty(Object bean, String name)
        ...
        // Call the property getter and return the value
        Object value = invokeMethod(readMethod, bean, EMPTY_OBJECT_ARRAY);
        //如果不处理,那么属性对象在构建时必须初始化,否则报空指针异常
        if(value == null){
        	try {
        		//当调用getter方法去得到属性时，返回null，则创建实例，通过该属性实例的setter方法把实例注入
        ...
        if (descriptor instanceof MappedPropertyDescriptor) {
          ...
          if (readMethod != null) {
            ...
            if (invokeResult instanceof java.util.Map) { 
				...
				Type valType = ((ParameterizedType)returnType).getActualTypeArguments()[1];					
				Object mapKey = ConvertUtils.convert(key,(Class<?>)keyType);
				Object mapValue = ConvertUtils.convert(value,(Class<?>)valType);
    ...
    public void setSimpleProperty(Object bean,
            ...
            throws IllegalAccessException, InvocationTargetException,
            NoSuchMethodException {
    	//System.out.println("target: "+bean+"   filed: "+name+"   value: "+value);
        if (bean == null) {
            throw new IllegalArgumentException("No bean specified");
        }
        if (name == null) {
    ...
    protected Object invokeMethod(
        ...
        try {
            ...
            //屏蔽此异常信息,并添加返回null值,映射组件会自动赋初始值
            //throw e;
        }
        return null;
    }
